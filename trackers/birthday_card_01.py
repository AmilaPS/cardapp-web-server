...
if __name__ == '__main__':
    ...
    caps = {
        "inside_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784372868429_Inside.mp4')),
        "butterfly_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784372868430_Butterfly.mp4')),
        "frontback_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784372868431_Front_Back.mp4')),
        "front_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784372868432_Front_Cover.mp4')),
    }
    imgs = {
        "table_img": cv2.imread(sys.argv[1], cv2.IMREAD_UNCHANGED),
        "inside_img": cv2.imread(sys.argv[2], cv2.IMREAD_UNCHANGED),
        "butterfly_img": cv2.imread(sys.argv[3], cv2.IMREAD_UNCHANGED),
        "frontback_img": cv2.imread(sys.argv[4], cv2.IMREAD_UNCHANGED),
        "front_img": cv2.imread(sys.argv[5], cv2.IMREAD_UNCHANGED),
    }
    
    if imgs["table_img"] is None: sys.exit(1)
    
    rendering_order = ["inside_img", "butterfly_img", "frontback_img", "front_img"]
