...
if __name__ == '__main__':
    ...
    caps = {
        "in_art_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784202388152_Inside_Artwork.mp4')),
        "fl_ca_ba_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784202388153_Flip_Card_Back.mp4')),
        "fl_ca_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784202388154_Flip_Card.mp4')),
        "fr_pa_ba_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784202388154_Front_Page_Back.mp4')),
        "fr_pa_img": cv2.VideoCapture(os.path.join(templates_dir, 'tmpl_1784202388155_Front_Page.mp4')),
    }
    imgs = {
        "back_img": cv2.imread(sys.argv[1], cv2.IMREAD_UNCHANGED),
        "in_art_img": cv2.imread(sys.argv[2], cv2.IMREAD_UNCHANGED),
        "fl_ca_ba_img": cv2.imread(sys.argv[3], cv2.IMREAD_UNCHANGED),
        "fl_ca_img": cv2.imread(sys.argv[4], cv2.IMREAD_UNCHANGED),
        "fr_pa_ba_img": cv2.imread(sys.argv[5], cv2.IMREAD_UNCHANGED),
        "fr_pa_img": cv2.imread(sys.argv[6], cv2.IMREAD_UNCHANGED),
    }
    
    if imgs["back_img"] is None: sys.exit(1)
    
    rendering_order = ["in_art_img", "fl_ca_ba_img", "fl_ca_img", "fr_pa_ba_img", "fr_pa_img"]
